    ...
    while from_timestamp < end_timestamp and (int(time.time()*1000) > from_timestamp):
        ...
        try:
            ...
            ohlcvs = exchange.fetch_ohlcv(
                symbol, timeframe, from_timestamp, limit)
            # within the from_timestamp > ohlcvs > end range
            if (ohlcvs[0][0] > end_timestamp) or (ohlcvs[-1][0] > end_timestamp):
                print("got a candle out of range! start:%s;  end:%s: end_timestamp:%s" % (
                    ohlcvs[0][0], ohlcvs[-1][0], end_timestamp))
                break
            # ---------------------------------------------------------------------
            # 更新from_timestamp
            ...
            # 追加新获取到的data
            data += ohlcvs
            print(str(len(data)) + ' of ' +
                  str(int(candle_no)) + ' candles loaded...')
